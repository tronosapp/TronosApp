 ...
 *
 * FIX v3: Se agregó tracking de pendingDeletes para detectar automáticamente
 * qué IDs fueron eliminados por el usuario (comparando estado viejo vs nuevo
 * en localStorage). Estos IDs se envían al servidor como `deletedIds` para
 * que el endpoint /api/sync pueda borrarlos explícitamente sin afectar los
...
(function() {
  ...
  // Cuando el usuario borra un registro, detectamos el ID "desaparecido"
  // comparando el estado viejo con el nuevo en localStorage.
  // Estos IDs se acumulan aquí y se envían al servidor en el próximo sync.
  // =====================================================================
  const pendingDeletes = {
    services:      new Set(), // IDs de servicios eliminados
    folders:       new Set(), // IDs de carpetas eliminadas
    preprogrammed: new Set()  // IDs de preprogramados eliminados
  };

  // Mapeo de clave de localStorage → categoría de pendingDeletes
    ...
    if (SYNC_KEYS.includes(key)) {
      try {
        ...
        if (Array.isArray(oldArr) && Array.isArray(newArr)) {
          ...
          // IDs que estaban antes y ya no están → fueron eliminados
          oldArr.forEach(item => {
          ...
          // Si un ID vuelve a aparecer (ej: deshacer), sacarlo de pendingDeletes
          newArr.forEach(item => {
            if (item.id != null) {
        ...
        // Silenciar errores de parseo para no interrumpir el flujo de la app
      }
    }

    // Llamar al original (mantener localStorage funcional)
  ...
  async function syncToServer() {
    ...
    // Capturar snapshot de pendingDeletes ANTES del fetch (por si llega otro
    // cambio mientras estamos sincronizando)
    ...
    try {
      ...
      const totalDeletes = deletedIdsSnapshot.services.length
        + deletedIdsSnapshot.folders.length
        + deletedIdsSnapshot.preprogrammed.length;

      console.log(
        `🔄 Sincronizando con servidor (Push): ` +
        `${databaseData.length} servicios, ` +
        `${preprogrammedData.length} pre-registros, ` +
        `${foldersData.length} carpetas` +
        (totalDeletes > 0 ? `, ${totalDeletes} eliminación(es)` : '')
      );
      ...
      const response = await fetch(`${API_URL}/api/sync`, {
        ...
        body: JSON.stringify({
          ...
          folders:       foldersData,
          deletedIds:    deletedIdsSnapshot  // ← IDs a borrar explícitamente en el servidor
        })
      });

      if (response.ok) {
        // Limpiar las eliminaciones pendientes solo tras confirmación del servidor
        ...
        // No limpiamos pendingDeletes para reintentar en el próximo sync
      }
    } catch (error) {
