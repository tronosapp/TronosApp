...
app.post('/api/sync', async (req, res) => {
  ...
  try {
    ...
    for (const p of preprogrammed) {
      await connection.query(
        'INSERT INTO preprogrammed (id,fecha,pago,cliente,contacto,origen,status) VALUES (?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE fecha=VALUES(fecha),pago=VALUES(pago),cliente=VALUES(cliente),contacto=VALUES(contacto),origen=VALUES(origen),status=VALUES(status)',
        [p.id, p.fecha||null, p.pago||'Efectivo', p.cliente||'', p.contacto||null, p.origen||null, p.status||'PRE']
      );
    }
